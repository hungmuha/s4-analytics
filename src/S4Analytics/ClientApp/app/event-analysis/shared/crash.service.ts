...
export class CrashService {
    ...
    createCrashQuery(query: CrashQuery): Observable<CrashQueryRef> {
        let url = 'api/crash/query';
        return this.http
            .post(url, query)
            .map(response => response.json() as CrashQueryRef);
    }

    getCrashFeatures(queryToken: string, extent: ol.Extent): Observable<EventFeatureSet> {
        let minX = extent[0];
        let minY = extent[1];
        let maxX = extent[2];
        let maxY = extent[3];
    ...
    getCrashData(queryToken: string, fromIndex: number, toIndex: number): Observable<Array<CrashResult>> {
